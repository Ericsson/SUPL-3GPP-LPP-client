...
### Fixed
- `example-client`: `--ls-horacc` now rejects values outside the documented 0-127 range instead of silently accepting them
...
- `tokoro-post`: new standalone SSR→VRS post-processing binary; merge-heap replay of UBX + SSR tbins with RINEX nav, no scheduler or streamline overhead
...
### Fixed
- SPARTN generator: BDS SF100 IODC now computed as `mod(toe_sec / 720, 240)` from LPP `iod_r15` (was incorrectly emitting raw `iod_r15 & 0xFF`). LPP encodes `iod_r15` as the 11 MSBs of `toe` (scale 512s) for B1I/B3I; the RTCM/CSNO-agreed algorithm requires `IODC = mod(toc / 720, 240)`.
