...
static int
per__max_range(intmax_t lb, intmax_t ub, uintmax_t *range_r) {
...
int
per_max_range_rebase(intmax_t v, intmax_t lb, intmax_t ub, uintmax_t *output) {
    uintmax_t range;

    assert(lb <= ub);

    if(v < lb || v > ub || per__max_range(lb, ub, &range) < 0) {
        /* Range error. */
        return -1;
    }
...
int
per_max_range_unrebase(uintmax_t inp, intmax_t lb, intmax_t ub, intmax_t *outp) {
    uintmax_t range;

    if(per__max_range(lb, ub, &range) != 0) {
        return -1;
    }
    ...
    if(inp <= INTMAX_MAX) {
        ...
        *outp = (lb + INTMAX_MAX + 1) + (long)((inp - INTMAX_MAX) - 1);
    }
