...
static void parse_interface(std::string const& source, InputsConfig& config) {
    ...
    if (type == "stdin") return parse_stdin(options, config);
    ...
    if (type == "udp-server") return parse_udp_server(options, config);
