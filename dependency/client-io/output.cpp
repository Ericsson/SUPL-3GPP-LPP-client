...
static void parse_interface(std::string const& source, OutputsConfig& config) {
    ...
    if (type == "stdout") return parse_stdout(options, config);
    if (type == "file") return parse_file(options, config);
    if (type == "chunked-log") return parse_chunked_log(options, config);
    if (type == "tcp-server") return parse_tcp_server(options, config);
    if (type == "stream") return parse_stream_ref(options, config);
    if (type == "serial") return parse_serial(options, config);
    if (type == "tcp-client") return parse_tcp_client(options, config);
    if (type == "udp-client") return parse_udp_client(options, config);
