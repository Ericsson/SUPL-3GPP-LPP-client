...
Ned<Frame> ecef_to_ned(Ecef<Frame> const& ecef, Llh<Frame> const& origin_llh) {
    ...
    return Ned<Frame>{ecef_to_ned_impl(ecef.value, origin_llh.value, Frame::ellipsoid)};
}
...
Ecef<Frame> ned_to_ecef(Ned<Frame> const& ned, Llh<Frame> const& origin_llh) {
    ...
    return Ecef<Frame>{ned_to_ecef_impl(ned.value, origin_llh.value, Frame::ellipsoid)};
}
