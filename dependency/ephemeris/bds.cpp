...
#define LOGLET_CURRENT_MODULE "eph/bds"
...
double BdsEphemeris::calculate_elapsed_time(ts::Bdt const& time, double reference) const NOEXCEPT {
    ...
    auto reference_ts =
        ts::Bdt::from_week_tow(time.week(), reference_tow, reference_frac).timestamp();
    auto current_ts = time.timestamp();
...
double BdsEphemeris::calculate_elapsed_time_toe(ts::Bdt const& time) const NOEXCEPT {
    return calculate_elapsed_time(time, toe);
}

double BdsEphemeris::calculate_elapsed_time_toc(ts::Bdt const& time) const NOEXCEPT {
    return calculate_elapsed_time(time, toc);
}

double BdsEphemeris::calculate_eccentric_anomaly(double t_k) const NOEXCEPT {
