...
CONSTEXPR static double GAL_CONSTANT_MU              = 3.986004418e14;
CONSTEXPR static double GAL_CONSTANT_OMEGA_EARTH_DOT = 7.2921151467e-5;
...
double GalEphemeris::calculate_eccentric_anomaly(double t_k) const NOEXCEPT {
    ...
    auto n0  = std::sqrt(GAL_CONSTANT_MU / std::pow(a, 3));
    auto n   = n0 + delta_n;
    auto m_k = m0 + n * t_k;
    VERBOSEF("a: %f", a);
    VERBOSEF("n0: %f", n0);
...
double GalEphemeris::calculate_eccentric_anomaly_rate(double e_k) const NOEXCEPT {
    auto n0 = std::sqrt(GAL_CONSTANT_MU / (a * a * a));
    auto n  = n0 + delta_n;
...
double GalEphemeris::calculate_relativistic_correction(Float3 const& position,
    ...
    auto t_r = -2.0 * r_v / (GAL_CONSTANT_C * GAL_CONSTANT_C);
    VERBOSEF("t_r: %+.14f (%+.14fm)", t_r, t_r * GAL_CONSTANT_C);
    return t_r;
}
...
double GalEphemeris::calculate_relativistic_correction_idc(double e_k) const NOEXCEPT {
    ...
    auto delta_t =
        -2.0 * e_sin * e * std::sqrt(a * GAL_CONSTANT_MU) / (GAL_CONSTANT_C * GAL_CONSTANT_C);
...
EphemerisResult GalEphemeris::compute(ts::Gst const& time) const NOEXCEPT {
    ...
    auto omega_k = omega0 + (omega_dot - GAL_CONSTANT_OMEGA_EARTH_DOT) * t_k -
                   GAL_CONSTANT_OMEGA_EARTH_DOT * toe;
    auto dot_omega_k = omega_dot - GAL_CONSTANT_OMEGA_EARTH_DOT;
