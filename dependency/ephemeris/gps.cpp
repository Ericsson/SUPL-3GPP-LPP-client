...
#define LOGLET_CURRENT_MODULE "eph"
...
double GpsEphemeris::calculate_elapsed_time_toe(ts::Gps const& time) const NOEXCEPT {
    return calculate_elapsed_time(time, toe);
}

double GpsEphemeris::calculate_elapsed_time_toc(ts::Gps const& time) const NOEXCEPT {
    return calculate_elapsed_time(time, toc);
}

double GpsEphemeris::calculate_eccentric_anomaly(double t_k) const NOEXCEPT {
