...
CONSTEXPR static double QZS_CONSTANT_MU              = 3.986005e14;
CONSTEXPR static double QZS_CONSTANT_OMEGA_EARTH_DOT = 7.2921151467e-5;
CONSTEXPR static double QZS_CONSTANT_C               = 2.99792458e8;

bool QzsEphemeris::is_valid(ts::Gps const& time) const NOEXCEPT {
...
double QzsEphemeris::calculate_elapsed_time(ts::Gps const& time, double reference) const NOEXCEPT {
...
double QzsEphemeris::calculate_elapsed_time_toe(ts::Gps const& time) const NOEXCEPT {
    return calculate_elapsed_time(time, toe);
}

double QzsEphemeris::calculate_elapsed_time_toc(ts::Gps const& time) const NOEXCEPT {
    return calculate_elapsed_time(time, toc);
}

double QzsEphemeris::calculate_eccentric_anomaly(double t_k) const NOEXCEPT {
    VSCOPE_FUNCTIONF("%f", t_k);

    auto n0  = std::sqrt(QZS_CONSTANT_MU / std::pow(a, 3));
    auto n   = n0 + delta_n;
    auto m_k = m0 + n * t_k;
    VERBOSEF("a: %f", a);
    VERBOSEF("n0: %f", n0);
...
double QzsEphemeris::calculate_eccentric_anomaly_rate(double e_k) const NOEXCEPT {
    auto n0 = std::sqrt(QZS_CONSTANT_MU / (a * a * a));
    auto n  = n0 + delta_n;
    return n / (1 - e * std::cos(e_k));
}

double QzsEphemeris::calculate_clock_bias(ts::Gps const& time) const NOEXCEPT {
...
double QzsEphemeris::calculate_relativistic_correction(Float3 const& position,
    ...
    auto t_r = -2.0 * r_v / (QZS_CONSTANT_C * QZS_CONSTANT_C);
    VERBOSEF("t_r: %+.14f (%+.14fm)", t_r, t_r * QZS_CONSTANT_C);
    return t_r;
}

double QzsEphemeris::calculate_relativistic_correction_idc(double e_k) const NOEXCEPT {
    FUNCTION_SCOPE();

    auto e_sin = std::sin(e_k);
    auto delta_t =
        -2.0 * e_sin * e * std::sqrt(a * QZS_CONSTANT_MU) / (QZS_CONSTANT_C * QZS_CONSTANT_C);
    VERBOSEF("delta_t: %+.14f", delta_t);

    return delta_t;
}

double QzsEphemeris::calculate_group_delay() const NOEXCEPT {
    return tgd;
}

EphemerisResult QzsEphemeris::compute(ts::Gps const& time) const NOEXCEPT {
    VSCOPE_FUNCTIONF("%s", ts::Utc(time).rtklib_time_string().c_str());
    ...
    auto omega_k = omega0 + (omega_dot - QZS_CONSTANT_OMEGA_EARTH_DOT) * t_k -
                   QZS_CONSTANT_OMEGA_EARTH_DOT * toe;
    auto dot_omega_k = omega_dot - QZS_CONSTANT_OMEGA_EARTH_DOT;
