...
std::unique_ptr<Message> Rtcm1042::parse(std::vector<uint8_t> data) {
    if (data.size()*8 < 8+16+512+24) {
        ERRORF("RTCM 1042 message created without enough data (requires %d bits, received %d bits)", 8+16+512+24, data.size()*8);
    ...
    if (m->mType != 1042) {
        ERRORF("RTCM 1042 message missmatched message number. should be '1042', was '%4d'", m->mType.value());
