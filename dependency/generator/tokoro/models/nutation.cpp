...
Nutation Nutation::evaluate(double t, AstronomicalArguments const& args) NOEXCEPT {
    ...
    // 0.1 mas to rad
    result.d_psi *= 1.0e-4 * constant::ARCSEC2RAD;
    result.d_eps *= 1.0e-4 * constant::ARCSEC2RAD;
    VERBOSEF("d_psi: %+.14f", result.d_psi);
    VERBOSEF("d_eps: %+.14f", result.d_eps);
    return result;
}
