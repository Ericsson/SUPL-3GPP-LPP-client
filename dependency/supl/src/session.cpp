...
bool Session::send(const START& message) {
    ...
    return mTcpClient->send(encoded_message.data(), encoded_message.size()) ==
...
bool operator==(const Identity& A, const Identity& B) {
...
static bool operator==(const Session::SET& A, const Session::SET& B) {
    if (A.id != B.id) return false;
    if (A.is_active != B.is_active) return false;
    return A.identity == B.identity;
}

static bool operator==(const Session::SLP& A, const Session::SLP& B) {
