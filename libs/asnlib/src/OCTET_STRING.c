...
OCTET_STRING_per_get_characters(asn_per_data_t *po, uint8_t *buf,
		...
		intmax_t lb, intmax_t ub, const asn_per_constraints_t *pc) {
	uint8_t *end = buf + units * bpc;

	ASN_DEBUG("Expanding %d characters into (%"ASN_PRIdMAX"..%"ASN_PRIdMAX"):%d",
		(int)units, lb, ub, unit_bits);

	/* X.691: 27.5.4 */
	if((uintmax_t)ub <= ((uintmax_t)2 << (unit_bits - 1))) {
		...
		for(; buf < end; buf += bpc) {
			...
			if(value < 0) {
				ASN_DEBUG("Code %d (0x%02x) is"
					" not in map (%"ASN_PRIdMAX"..%"ASN_PRIdMAX")",
					code, code, lb, ub);
				return 1;	/* FATAL */
			}
			switch(bpc) {
	...
	for(; buf < end; buf += bpc) {
		...
		if(code < 0) return -1;	/* WMORE */
		if(ch > ub) {
			ASN_DEBUG("Code %d is out of range (%"ASN_PRIdMAX"..%"ASN_PRIdMAX")",
				ch, lb, ub);
			return 1;	/* FATAL */
		}
		switch(bpc) {
...
OCTET_STRING_per_put_characters(asn_per_outp_t *po, const uint8_t *buf,
	...
	ASN_DEBUG("Squeezing %d characters into (%"ASN_PRIdMAX"..%"ASN_PRIdMAX"):%d (%d bpc)",
    ...
    for(ub -= lb; buf < end; buf += bpc) {
        ...
        ch = value - lb;
        if(ch < 0 || ch > ub) {
            ASN_DEBUG("Character %d (0x%02x) is out of range (%"ASN_PRIdMAX"..%"ASN_PRIdMAX")", *buf,
                      value, lb, ub + lb);
            return -1;
        }
