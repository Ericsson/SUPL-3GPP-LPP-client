...
int to_msm_lock_ex(double lock)
{
...
static int encode_type1005(rtcm_t *rtcm, int sync)
{
    ...
    set38bits(rtcm->buff,i,p[0]*10000.0 ); i+=38; /* antenna ref point ecef-x */
    setbitu(rtcm->buff,i, 1,1          ); i+= 1; /* oscillator indicator */
    setbitu(rtcm->buff,i, 1,0          ); i+= 1; /* reserved */
    set38bits(rtcm->buff,i,p[1]*10000.0 ); i+=38; /* antenna ref point ecef-y */
    setbitu(rtcm->buff,i, 2,0          ); i+= 2; /* quarter cycle indicator */
    set38bits(rtcm->buff,i,p[2]*10000.0 ); i+=38; /* antenna ref point ecef-z */
    rtcm->nbit=i;
    return 1;
}
...
static int encode_type1006(rtcm_t *rtcm, int sync)
{
    ...
    setbitu(rtcm->buff,i,12,rtcm->staid); i+=12; /* ref station id */
    setbitu(rtcm->buff,i, 6,0          ); i+= 6; /* itrf realization year */
    setbitu(rtcm->buff,i, 1,rtcm->sta.gnss_indicator_gps?1:0          ); i+= 1; /* gps indicator */
    setbitu(rtcm->buff,i, 1,rtcm->sta.gnss_indicator_glo?1:0          ); i+= 1; /* glonass indicator */
    setbitu(rtcm->buff,i, 1,rtcm->sta.gnss_indicator_gal?1:0          ); i+= 1; /* galileo indicator */
    setbitu(rtcm->buff,i, 1,rtcm->sta.rs_indicator?1:0          ); i+= 1; /* ref station indicator */
    set38bits(rtcm->buff,i,p[0]*10000.0 ); i+=38; /* antenna ref point ecef-x */
    setbitu(rtcm->buff,i, 1,rtcm->sta.sro_indicator?1:0          ); i+= 1; /* oscillator indicator */
    setbitu(rtcm->buff,i, 1,0          ); i+= 1; /* reserved */
    set38bits(rtcm->buff,i,p[1]*10000.0 ); i+=38; /* antenna ref point ecef-y */
    setbitu(rtcm->buff,i, 2,rtcm->sta.qc_indicator?1:0          ); i+= 2; /* quarter cycle indicator */
    set38bits(rtcm->buff,i,p[2]*10000.0 ); i+=38; /* antenna ref point ecef-z */
...
/* encode type 1030: gnss residuals -------------------------*/
static int encode_type1030_1031(rtcm_t *rtcm, int sys, int sync)
{
    int i=24, week; 
    double tow;
    
    trace(3,"encode_type1030_1031: sync=%d, sys=%d\n",sync, sys);
    ...
    if (sys==SYS_GLO) {
        ...
        printf("Unsupported sys %d received in function encode_type1030_1031\n", sys);
        return 0;
    }

    rtklib_residuals_t residuals = rtcm->residuals;

    if (sys == SYS_GPS) {
        setbitu(rtcm->buff,i,12,1030                        ); i+=12;
        setbitu(rtcm->buff,i,20,tow                         ); i+=20; /* tow */
    } else if(sys == SYS_GLO) {
        setbitu(rtcm->buff,i,12,1031                        ); i+=12;
        setbitu(rtcm->buff,i,17,tow                         ); i+=17; /* tow */
    }
    
    
    setbitu(rtcm->buff,i,12,rtcm->staid                 ); i+=12; /* reference station id */
    setbitu(rtcm->buff,i,7,residuals.n_refs             ); i+=7;  /* n refs */
    setbitu(rtcm->buff,i,5,residuals.n_sat_sig_processed); i+=5;  /* n refs */


    for(int j=0; j<residuals.n_sat_sig_processed; j++) {
        rtklib_residual_element_t ele = residuals.list[j];
        setbitu(rtcm->buff,i,6,ele.satellite_id); i+= 6;
        setbitu(rtcm->buff,i,8,ele.soc         ); i+= 8;
        setbitu(rtcm->buff,i,9,ele.sod         ); i+= 9;
        setbitu(rtcm->buff,i,6,ele.soh         ); i+= 6;
        setbitu(rtcm->buff,i,10,ele.slc        ); i+= 10;
        setbitu(rtcm->buff,i,10,ele.sld        ); i+= 10;
    }
    rtcm->nbit=i;
    return 1;
}
/* encode type 1032: physical reference station position --------*/
static int encode_type1032(rtcm_t *rtcm, int sync)
{
    int i=24;
    phy_sta_t phy_sta=rtcm->phy_sta;


    trace(3,"encode_type1032: sync=%d\n",sync);

    double *p=phy_sta.pos;

    setbitu(rtcm->buff,i,12,1032                        ); i+=12; /* message no */
    setbitu(rtcm->buff,i,12,phy_sta.non_physical_sta_id ); i+=12; /* ref station id */
    setbitu(rtcm->buff,i,12,phy_sta.physical_sta_id     ); i+=12; /* ref station id */
    setbitu(rtcm->buff,i, 6,phy_sta.itrf                ); i+= 6; /* itrf realization year */
    set38bits(rtcm->buff,i,p[0]*10000.0                  ); i+=38; /* antenna ref point ecef-x */
    set38bits(rtcm->buff,i,p[1]*10000.0                  ); i+=38; /* antenna ref point ecef-y */
    set38bits(rtcm->buff,i,p[2]*10000.0                  ); i+=38; /* antenna ref point ecef-z */
    rtcm->nbit=i;
    ...
    *nsat=*nsig=*ncell=0;
...
#if 0
    printf("msm-satellites (%i):", *nsat);
    for (i=0;i<64;i++) {
        if (sat_ind[i]) printf(" %2i", i+1);
    }
    printf("\n");
    printf("msm-signals (%i):", *nsig);
    for (i=0;i<32;i++) {
        if (sig_ind[i]) printf(" %2i", i+1);
    }
    printf("\n");
#endif
    ...
    for (i=0;i<rtcm->obs.n;i++) {
        ...
        for(j=0;j<*nsig;j++) {
            ...
            if (cell_ind[k]) {
                for (int y=0;y<32;y++) {
                    if (j+1 == sig_ind[y]) printf(" %2i", y+1);
                }
            } else {
                printf(" --");
            }
    ...
    for (j=0;j<(*nsat)*(*nsig)&&j<64;j++) {
        printf(" %i", cell_ind[j]?1:0);
        if(j % (*nsig) == (*nsig) - 1) printf("\n");
    }
    printf("\n");
#endif
}
...
static void gen_msm_sat(rtcm_t *rtcm, int sys, int nsat, const uint8_t *sat_ind,
                        OptDouble *rrng, OptDouble* rrng_mod, OptDouble *rrate, uint8_t *info)
{
    obsd_t *data;
    double freq;
   ...
   for (i=0;i<64;i++) {
        if(rrng) rrng[i].available = 0;
        if(rrng_mod) rrng_mod[i].available = 0;
        if(rrate) rrate[i].available = 0;
        if(info) info[i]=0;
    }
    ...
    for (i=0;i<rtcm->obs.n;i++) {
        ...
        for (j=0;j<NFREQ+NEXOBS;j++) {
            ...
            if(rrng) rrng[k] = data->integer_ms[j];
            if(rrng) rrng_mod[k] = data->rough_range_modulo_ms[j];
            if(rrate) rrate[k] = data->rough_phase_range_rates[j];
            if(info) info[k] = data->extended_satellite_info[j];
        }
    }
}
...
static void gen_msm_sig(rtcm_t *rtcm, int sys, int nsat, int nsig, int ncell,
                        ...
                        const uint8_t *cell_ind, const OptDouble *rrng,
                        const OptDouble *rrate, OptDouble *psrng, OptDouble *phrng,
                        OptDouble *rate, OptDouble *lock, uint8_t *half, OptDouble *cnr)
{
    obsd_t *data;
    ...
    for (i=0;i<ncell;i++) {
        if (psrng) psrng[i].available = 0;
        if (phrng) phrng[i].available = 0;
        if (rate ) rate [i].available = 0;
        if (cnr ) cnr [i].available = 0;
    }
    ...
    for (i=0;i<rtcm->obs.n;i++) {
        ...
        for (j=0;j<NFREQ+NEXOBS;j++) {
            ...
            if(psrng) psrng[cell-1] = data->fine_pseudo_range[j];
            if(phrng) phrng[cell-1] = data->fine_phase_range[j];
            if(rate) rate[cell-1] = data->fine_phase_range_rates[j];
            if(lock) lock[cell-1] = data->phase_range_lock[j];
            if(half) half[cell-1] = data->half_cycle[j];
            if(cnr) cnr[cell-1] = data->carrier_to_noise_ratio[j];
        }
    }
}
/* encode MSM header ---------------------------------------------------------*/
static int encode_msm_head(int type, rtcm_t *rtcm, int sys, int sync, int *nsat,
                           int *ncell, OptDouble *rrng, OptDouble* rrng_mod, OptDouble *rrate,
                           uint8_t *info, OptDouble *psrng, OptDouble *phrng,
                           OptDouble *rate, OptDouble *lock, uint8_t *half,
                           OptDouble *cnr)
{
    double tow;
    ...
    setbitu(rtcm->buff,i, 7,0          ); i+= 7; /* reserved */
    setbitu(rtcm->buff,i, 2,rtcm->clock_steering.available?(rtcm->clock_steering.value&0x3):0          ); i+= 2; /* clock streering indicator */
    setbitu(rtcm->buff,i, 2,rtcm->external_clock.available?(rtcm->external_clock.value&0x3):0          ); i+= 2; /* external clock indicator */
    setbitu(rtcm->buff,i, 1,rtcm->smooth_indicator.available?(rtcm->smooth_indicator.value&0x1):0          ); i+= 1; /* smoothing indicator */
    setbitu(rtcm->buff,i, 3,rtcm->smooth_interval.available?(rtcm->smooth_interval.value&0x7):0          ); i+= 3; /* smoothing interval */
    ...
    for (j=0;j<(*nsat);j++) {
        for(int k = 0; k < nsig; k++) {
            int idx = j*nsig + nsig - 1 - k; 
            setbitu(rtcm->buff,i,1,cell_ind[idx]?1:0); i+=1;
        }
    }
#endif

    /* generate msm satellite data fields */
    gen_msm_sat(rtcm,sys,*nsat,sat_ind,rrng,rrng_mod,rrate,info);
...
static int encode_msm_int_rrng(rtcm_t *rtcm, int i, const OptDouble *rrng,
                               int nsat)
{
    uint32_t int_ms;
    int j;
    ...
    for (j=0;j<nsat;j++) {
        if (!rrng[j].available||rrng[j].value==0.0) {
            int_ms=255;
        }
        else {
            int_ms=ROUND_U(rrng[j].value);
        }
...
static int encode_msm_mod_rrng(rtcm_t *rtcm, int i, const OptDouble *rrng_mod,
                               int nsat)
{
    uint32_t mod_ms;
    int j;
    ...
    for (j=0;j<nsat;j++) {
        if (!rrng_mod[j].available) {
            mod_ms=0;
        }
        else {
            mod_ms=ROUND_U(rrng_mod[j].value/P2_10)&0x3FFu;
        }
...
static int encode_msm_rrate(rtcm_t *rtcm, int i, const OptDouble *rrate, int nsat)
{
    int j,rrate_val;
    
    for (j=0;j<nsat;j++) {
        if (!rrate[j].available|| fabs(rrate[j].value)>8191.0) {
            trace(2,"msm rough phase-range-rate overflow %s rrate=%.4f\n",
                 time_str(rtcm->time,0),rrate[j].value);
            rrate_val=-8192;
        }
        else {
            rrate_val=ROUND(rrate[j].value/1.0);
        }
...
static int encode_msm_psrng(rtcm_t *rtcm, int i, const OptDouble *psrng, int ncell)
{
    int j,psrng_val;
    
    for (j=0;j<ncell;j++) {
        if (!psrng[j].available) {
            psrng_val=-16384;
        }
        else {
            psrng_val=ROUND(psrng[j].value/P2_24);
        }
...
static int encode_msm_psrng_ex(rtcm_t *rtcm, int i, const OptDouble *psrng,
                               int ncell)
{
    int j,psrng_val;
    ...
    for (j=0;j<ncell;j++) {
        if (!psrng[j].available) {
            ...
            psrng_val=ROUND(psrng[j].value/P2_29);
        }
...
static int encode_msm_phrng(rtcm_t *rtcm, int i, const OptDouble *phrng, int ncell)
{
    int j,phrng_val;
    
    for (j=0;j<ncell;j++) {
        if (!phrng[j].available) {
            phrng_val=-2097152;
        }
        else {
            phrng_val=ROUND(phrng[j].value/P2_29);
        }
...
static int encode_msm_phrng_ex(rtcm_t *rtcm, int i, const OptDouble *phrng,
                               int ncell)
{
    int j,phrng_val;
    ...
    for (j=0;j<ncell;j++) {
        if (!phrng[j].available) {
            ...
            phrng_val=ROUND(phrng[j].value/P2_31);
        }
...
static int encode_msm_lock(rtcm_t *rtcm, int i, const OptDouble *lock, int ncell)
{
    int j,lock_val;
    
    for (j=0;j<ncell;j++) {
        lock_val=to_msm_lock(lock[j].available?lock[j].value:0);
        setbitu(rtcm->buff,i,4,lock_val); i+=4;
    }
    return i;
}
/* encode lock-time indicator with extended range and resolution -------------*/
static int encode_msm_lock_ex(rtcm_t *rtcm, int i, const OptDouble *lock,
                              int ncell)
{
    int j,lock_val;
    
    for (j=0;j<ncell;j++) {
        lock_val=to_msm_lock_ex(lock[j].available?lock[j].value:0);
...
/* encode signal CNR ---------------------------------------------------------*/
static int encode_msm_cnr(rtcm_t *rtcm, int i, const OptDouble *cnr, int ncell)
{
    int j,cnr_val;
    
    for (j=0;j<ncell;j++) {
        cnr_val=ROUND((cnr[j].available?cnr[j].value:0)/1.0);
        setbitu(rtcm->buff,i,6,cnr_val); i+=6;
    }
    return i;
}
/* encode signal CNR with extended resolution --------------------------------*/
static int encode_msm_cnr_ex(rtcm_t *rtcm, int i, const OptDouble *cnr, int ncell)
{
    int j,cnr_val;
    
    for (j=0;j<ncell;j++) {
        cnr_val=ROUND((cnr[j].available?cnr[j].value:0)/0.0625);
        setbitu(rtcm->buff,i,10,cnr_val); i+=10;
    }
    return i;
}
/* encode fine phase-range-rate ----------------------------------------------*/
static int encode_msm_rate(rtcm_t *rtcm, int i, const OptDouble *rate, int ncell)
{
    int j,rate_val;
    
    for (j=0;j<ncell;j++) {
        if (!rate[j].available|| rate[j].value==0.0) {
            rate_val=-16384;
        }
        else {
            rate_val=ROUND(rate[j].value/0.0001);
        }
...
{
    OptDouble rrng[64],rrng_mod[64],rrate[64],psrng[64];
    int i,nsat,ncell;
    
    trace(3,"encode_msm1: sys=%d sync=%d\n",sys,sync);
    
    /* encode msm header */
    if (!(i=encode_msm_head(1,rtcm,sys,sync,&nsat,&ncell,rrng,rrng_mod,rrate,NULL,psrng,
                            NULL,NULL,NULL,NULL,NULL))) {
        return 0;
    }
    /* encode msm satellite data */
    i=encode_msm_mod_rrng(rtcm,i,rrng_mod ,nsat ); /* rough range modulo 1 ms */
...
{
    OptDouble rrng[64],rrng_mod[64],rrate[64],phrng[64],lock[64];
    uint8_t half[64];
    int i,nsat,ncell;
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(2,rtcm,sys,sync,&nsat,&ncell,rrng,rrng_mod,rrate,NULL,NULL,
...
{
    OptDouble rrng[64],rrng_mod[64],rrate[64],psrng[64],phrng[64],lock[64];
    uint8_t half[64];
    int i,nsat,ncell;
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(3,rtcm,sys,sync,&nsat,&ncell,rrng,rrng_mod,rrate,NULL,psrng,
                            phrng,NULL,lock,half,NULL))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm4(rtcm_t *rtcm, int sys, int sync)
{
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(4,rtcm,sys,sync,&nsat,&ncell,rrng,rrng_mod,rrate,NULL,psrng,
...
{
    OptDouble rrng[64],rrng_mod[64],rrate[64],psrng[64],phrng[64],rate[64],lock[64];
    OptDouble cnr[64];
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(5,rtcm,sys,sync,&nsat,&ncell,rrng,rrng_mod,rrate,info,psrng,
...
static int encode_msm6(rtcm_t *rtcm, int sys, int sync)
{
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(6,rtcm,sys,sync,&nsat,&ncell,rrng,rrng_mod,rrate,NULL,psrng,
                            phrng,NULL,lock,half,cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm7(rtcm_t *rtcm, int sys, int sync)
{
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(7,rtcm,sys,sync,&nsat,&ncell,rrng,rrng_mod,rrate,info,psrng,
                            phrng,rate,lock,half,cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
extern int encode_rtcm3(rtcm_t *rtcm, int type, int subtype, int sync)
{
    ...
    switch (type) {
        ...
        case 1030: ret=encode_type1030_1031(rtcm,SYS_GPS, sync);     break;
        case 1031: ret=encode_type1030_1031(rtcm,SYS_GLO, sync);     break;
        case 1032: ret=encode_type1032(rtcm,sync);     break;
