...
typedef struct {        /* observation data record */
    ...
    OptDouble integer_ms[NFREQ+NEXOBS];    /* (rrng) MSM 5 Satellite Data DF 397, integer ms in GNSS satellite rough ranges */
    unsigned int extended_satellite_info[NFREQ+NEXOBS]; /* (info) MSM 5 Satellite Data, extended satellite info */
    OptDouble rough_range_modulo_ms[NFREQ+NEXOBS]; /* MSM 5 Satellite data, DF398 GNSS satellite rough ranges modulo 1 ms */
    OptDouble rough_phase_range_rates[NFREQ+NEXOBS]; /* (rrate) MSM 5 Satellite data, DF399 satellite rough phase range rates */
    OptDouble fine_pseudo_range[NFREQ+NEXOBS]; /* (psrng) MSM 5 Signal data, DF400 GNSS signal fine pseudoranges */
    OptDouble fine_phase_range[NFREQ+NEXOBS]; /* (phrng) MSM 5 Signal data, DF401 GNSS signal fine phase range data */
    OptDouble phase_range_lock[NFREQ+NEXOBS]; /* (lock) MSM 5 signal data, DF402 GNSS phase range lock time indicator */
    unsigned char half_cycle[NFREQ+NEXOBS]; /* (half) MSM 5 signal data, DF420, Half-cycle ambiguity indicator */
    OptDouble fine_phase_range_rates[NFREQ+NEXOBS]; /* (rate) MSM 5 signal data, DF404, GNSS signal fine phase range rates */
    OptDouble carrier_to_noise_ratio[NFREQ+NEXOBS];
    OptInteger channel_nummer[NFREQ+NEXOBS];
} obsd_t;
...
typedef struct {
    int non_physical_sta_id;    /* Non Pysical Reference Station ID DF003 */
    int physical_sta_id;        /* Physical Reference Staiton ID DF226 */
    int itrf;                   /* ITRF Epoch Year DF021 */
    double pos[3];              /* physical reference station position (ecef) */
} phy_sta_t;
...
typedef struct {        /* station parameter type */
    ...
    int gnss_indicator_gps;
    int gnss_indicator_glo;
    int gnss_indicator_gal;
    int rs_indicator;
    int sro_indicator;
    int qc_indicator;
} sta_t;
...
typedef struct {
    ...
    long    slc;
    long    sld;
} rtklib_residual_element_t;

typedef struct {
    long    n_refs;
    long    n_sat_sig_processed;
    rtklib_residual_element_t list[32];
} rtklib_residuals_t;
...
typedef struct {        /* RTCM control struct type */
    ...
    OptInteger clock_steering;
    OptInteger external_clock;
    OptInteger smooth_indicator;
    OptInteger smooth_interval;

    rtklib_residuals_t residuals; /* GNSS residuals */
    phy_sta_t phy_sta;  /* physical reference station position info */
} rtcm_t;
...
EXPORT int to_msm_lock(double lock);
EXPORT int to_msm_lock_ex(double lock);
