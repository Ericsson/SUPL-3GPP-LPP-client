...
static bool parse_double(const std::string& token, double& cog) {
    try {
        cog = std::stod(token);
        return true;
    } catch (...) {
...
std::unique_ptr<VtgMessage> VtgMessage::parse(std::string prefix, const std::string& payload) {
    ...
    auto success = true;
    success &= parse_double(tokens[0], message->mTrueCourseOverGround);
    success &= parse_double(tokens[2], message->mMagneticCourseOverGround);
