...
def build_bds_eph(eph_data, prn):
    ...
    sat = satid2no(f"C{prn:02d}")
    eph.sat = sat
    eph.week = bdt_week
    eph.toes = toe_sow
    eph.toe = gpst2time(gps_week, toe_sow_gps)
    eph.toc = gpst2time(gps_week, toc_sow)

    eph.iode = int(toe_sow / 720) % 240
    eph.iodc = int(toc_sow / 720) % 240
    eph.sva = int(eph_data['SVacc'].values)
    eph.svh = int(eph_data['SatH1'].values)
    eph.code = 0
    eph.tgd[0] = float(eph_data['TGD1'].values)
    eph.tgd[1] = float(eph_data['TGD2'].values)
