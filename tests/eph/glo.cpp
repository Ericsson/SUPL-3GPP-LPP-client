    ...
    for (auto const& filename : files) {
        ...
        for (uint32_t i = 0; i < num_ephemerides; i++) {
            ...
            for (auto const& test : tests) {
                auto time = ts::Glo{ts::Gps{ts::Timestamp{test.gps_sec}}};

                CAPTURE(eph.slot_number);
                CAPTURE(test.offset);
                ...
                REQUIRE(doctest::Approx(result.position.x) == test.x);
                REQUIRE(doctest::Approx(result.position.y) == test.y);
                REQUIRE(doctest::Approx(result.position.z) == test.z);
                ...
                auto time2      = ts::Glo{ts::Timestamp{time.timestamp().seconds(), delta_time}};
                ...
                REQUIRE(doctest::Approx(clock_bias) == test.clock_bias);
