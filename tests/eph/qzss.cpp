...
TEST_CASE("QZSS ephemeris computation") {
    ...
    if (files.empty()) {
        WARN("No QZSS test data files found - skipping ephemeris computation tests");
        return;
    }
    ...
    for (auto const& filename : files) {
        ...
        for (uint32_t i = 0; i < num_ephemerides; i++) {
            ...
            for (auto const& test : tests) {
                int  week     = test.gps_sec / 604800;
                long tow      = test.gps_sec % 604800;
                ...
                auto time2   = ts::Gps{ts::Timestamp{gps_time.timestamp().seconds(), delta_time}};
